...
from app.data.sdoh_domains import SDOH_DOMAINS
from app.data.settings import INITIAL_SYSTEM_PROMPT, INITIAL_QA_EXAMPLES, MODEL, MAX_TOKENS, TEMPERATURE
...
def build_risk_factors(survey, messages):
    ...
    for question_obj in survey.values():
        # print(f">>> Getting risk factors for question '{question_obj['question']}' with answer '{question_obj['answer']}'...")
        ...
        # print(f">>> Risk factors received! ({response_message})")

        risk_factors = decode_risk_factors(response_message)

        question_obj['risk_factors'] = risk_factors

        # Update overall risk factors with new risk factors
